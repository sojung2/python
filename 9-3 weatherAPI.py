...
def loc_code_sch(loc_name):
    loc_code = pd.read_csv('./data/기상청위치코드.csv', encoding='utf-8')
...
def weather_api(startDt, endDt, stnIds, numOfRows=10):
    ...
    params ={'serviceKey' : 'YbaVeVBcF6WXTW72%2Bg43KCupoxxQ1J2ESO21d1NTK%2FPrYiiOsdtXNKXsJcOXJdDrwI7qu7a7ebrplkeWvcmFLQ%3D%3D', 
            'pageNo' : '1', 
            'numOfRows' : numOfRows, 
            'dataType' : 'JSON', 
            'dataCd' : 'ASOS', 
...
def weather_ext(startDT, endDT, stnIds):
    ...
    for i in (range(int(startDT[:4]), int(endDT[:4])+1)):
